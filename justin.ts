...
import type { BinaryNode } from './src/types.js'

binary('in', PREC_COMP); operator<BinaryNode>('in', (a, b) => b && (a = compile(a), b = compile(b), ctx => a(ctx) in b(ctx)))

// register !==, ===
binary('===', PREC_EQ); binary('!==', 9 as precedence)
operator<BinaryNode>('===', (a, b) => (a = compile(a), b = compile(b), ctx => a(ctx) === b(ctx)))
operator<BinaryNode>('!==', (a, b) => (a = compile(a), b = compile(b), ctx => a(ctx) !== b(ctx)))

// add nullish coalescing
binary('??', PREC_LOR)
operator<BinaryNode>('??', (a, b) => b && (a = compile(a), b = compile(b), ctx => a(ctx) ?? b(ctx)))
binary('??=', PREC_ASSIGN, true)
operator<BinaryNode>('??=', (a, b) => (b = compile(b), prop(a, (obj, path, ctx) => (obj[path] ??= b(ctx)))))
...
// add JS literals
token('undefined', PREC_ASSIGN, a => a ? err() : [, undefined])
token('NaN', PREC_ASSIGN, a => a ? err() : [, NaN])
token('null', PREC_ASSIGN, a => a ? err() : [, null])
